...
class LocalFrmHelper
{
    #region 单例模式
    //private static readonly object lockobject = new object();
    public static LocalFrmHelper Instance { get { return RunnerInstanceHelper.Create<LocalFrmHelper>(); } }
    /// <summary>
    /// 构造函数
    /// </summary>
    private LocalFrmHelper()
    {
    }
    #endregion
    ...
    {
        StartLogFileHelper.Instance.StartMsg($"获取当前路径中类库的界面……");
        ...
        foreach (var file in files)
        {
            ...
            if (frm != null)
            {
                StartLogFileHelper.Instance.StartMsg($"获取当前路径中类库成功");
                return frm;
            }
        }
        StartLogFileHelper.Instance.StartMsg($"获取当前路径中类库失败");
        return null;
    }
}
